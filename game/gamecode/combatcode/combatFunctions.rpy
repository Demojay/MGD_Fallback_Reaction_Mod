...
init python:
    ...
    def GetParalEnergyChange(player):
        ParaPerkboost = 1
        if difficulty == "Hard":
            ParaPerkboost = 1.5
        for perk in player.perks:
            ...
            while  p < len(perk.PerkType):
                if perk.PerkType[p] == "ParalysisAmp":
                    ParaPerkboost += perk.EffectPower[p]*0.01
                p += 1
        return (1+(player.statusEffects.paralysis.potency*(0.2*ParaPerkboost)))

    def GetParalFlatEnergyChange(player):
        ParaPerkboost = 1
        if difficulty == "Hard":
            ParaPerkboost = 1.5
        for perk in player.perks:
            ...
            while  p < len(perk.PerkType):
                if perk.PerkType[p] == "ParalysisAmp":
                    ParaPerkboost += perk.EffectPower[p]*0.01
                p += 1
        return ((player.statusEffects.paralysis.potency*(2*ParaPerkboost)))

    def GetParalFlatEnergyCostDisplay(player):
        if GetParalFlatEnergyChange(player) > 0:
            return "{color=#F8FF6A}Costs " + str(math.floor(GetParalFlatEnergyChange(player))) + " Energy.{/color} "
        else:
            return ""

    def GetParalFlatEnergyCostDisplayCheck(player):
        if GetParalFlatEnergyChange(player) > 0:
            return " {color=#F8FF6A}Cost " + str(math.floor(GetParalFlatEnergyChange(player))) + " Energy!{/color} "
        else:
            return ""
    ...
    def ClearNonPersistentEffects(player):
        ...
        plap = 0
        while plap < len(player.perks):
            p = 0
            while  p < len(player.perks[plap].PerkType):
                if player.perks[plap].PerkType[p] == "NonPersistentEffect":
                    player.giveOrTakePerk(player.perks[plap].name, -1)
                    plap -= 1
                    break
                p += 1
            plap += 1
    ...
    def getTotalBoost(attacker, move): #should be a way to cut down this function i think. Will keep in mind for later.
        ...
        for perk in attacker.perks:
            ...
            while  p < len(perk.PerkType):
                ...
                elif perk.PerkType[p] == "BreastBoost":
                    for each in move.skillTags:
                        checkTag = each
                        if checkTag == "Breasts":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "AssBoost":
                    for each in move.skillTags:
                        canGo = 0
                        checkTag = each
                ...
                elif perk.PerkType[p] == "NonPenAssBoost":
                    checkPen = 0
                    for each in move.skillTags:
                        checkTag = each
                        if checkTag == "Penetration":
                ...
                elif perk.PerkType[p] == "NonPenSexBoost":
                    checkPen = 0
                    for each in move.skillTags:
                        checkTag = each
                        if checkTag == "Penetration":
                ...
                elif perk.PerkType[p] == "PainBoost":
                    for each in move.skillTags:
                        checkTag = each
                        if checkTag == "Pain":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "SeductionBoost":
                    for each in move.skillTags:
                        checkTag = each
                        if checkTag == "Seduction":
                            Boost += perk.EffectPower[p]*0.01
                elif perk.PerkType[p] == "NonPenSeductionBoost":
                    checkPen = 0
                    for each in move.skillTags:
                        checkTag = each
                        if checkTag == "Penetration":
                ...
                elif perk.PerkType[p] == "MagicBoost":
                    for each in move.skillTags:
                        checkTag = each
                        if checkTag == "Magic":
                            Boost +=  perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "NonPenMagicBoost":
                    checkPen = 0
                    for each in move.skillTags:
                        checkTag = each
                        if checkTag == "Penetration":
                ...
                elif perk.PerkType[p] == "PenetrationBoost":
                    for each in move.skillTags:
                        checkTag = each
                        if checkTag == "Penetration":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "OralBoost":
                    for fetishE in move.fetishTags:
                        checkTag = fetishE
                        if checkTag == "Oral":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "MonstrousBoost":
                    for fetishE in move.fetishTags:
                        checkTag = fetishE
                        if checkTag == "Monstrous":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "FeetUseBoost":
                    for fetishE in move.fetishTags:
                        checkTag = fetishE
                        if checkTag == "Feet":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "BreastUseBoost":
                    for fetishE in move.fetishTags:
                        checkTag = fetishE
                        if checkTag == "Breasts":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "AssUseBoost":
                    for fetishE in move.fetishTags:
                        checkTag = fetishE
                        if checkTag == "Ass":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "LegUseBoost":
                    for fetishE in move.fetishTags:
                        checkTag = fetishE
                        if checkTag == "Legs":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "KissBoost":
                    for fetishE in move.fetishTags:
                        checkTag = fetishE
                        if checkTag == "Kissing":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "ForeplayBoost":
                    for fetishE in move.fetishTags:
                        checkTag = fetishE
                        if checkTag == "Foreplay":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "SexToyBoost":
                    for fetishE in move.fetishTags:
                        checkTag = fetishE
                        if checkTag == "Sex Toy":
                            Boost += perk.EffectPower[p]*0.01

                elif perk.PerkType[p] == "IndulgentBoost":
                    for fetishE in move.fetishTags:
                        checkTag = fetishE
                        if checkTag == "Indulgent":
                            Boost += perk.EffectPower[p]*0.01
                        
                elif perk.PerkType[p] == "ItemBonus":
...
label combatActionTurn:
    ...
    $ epOut = 0
    if attacker.species == "Player" and skillChoice.name != "Wait" and skillChoice.name != "Defend" and skillChoice.name != "Struggle":
        if GetParalFlatEnergyChange(player) > 0:
            if player.stats.ep < int(math.floor(GetParalFlatEnergyChange(player))):
                $ canUse = False
                $ epOut = 1
        if skillChoice.costType == "ep":
            if int(math.floor(skillChoice.cost*GetParalEnergyChange(player)+GetParalFlatEnergyChange(player))) > attacker.stats.ep:
                $ canUse = False
                $ epOut = 1
    ...
    if canUse == False:
        ...
        "[display!i]"
    elif attacker.statusEffects.restrained.duration > 0 and skillIsUsable(skillChoice, attacker) == False and skillChoice.statusEffect != "Escape"  and skillChoice.name != " " and skillChoice.name != "Defend" and skillChoice.name != "Struggle" and skillChoice.name != "Wait":
        ...
        if goAttack == 1:
            ...
            if attacker.species == "Player":
                if GetParalFlatEnergyChange(player) > 0 and skillChoice.name != "Wait" and skillChoice.name != "Defend" and skillChoice.name != "Struggle":
                    $ attacker.stats.ep -= int(math.floor(GetParalFlatEnergyChange(player)))
                if skillChoice.costType == "ep":
                    $ attacker.stats.ep -= int(math.floor(skillChoice.cost*GetParalEnergyChange(player)))
                ...
                if(skillChoice.skillType == "attack"):
                    ...
                    if(skillChoice.statusChance > 0 and finalDamage > 0):
                        ...
                        if holder[1] != "Skip":
                            ...
                            elif attacker.species == "Player" and stanceGo == "True":
                                call onHitPreTalk(defender, attacker, skillChoice) from _call_onHitPreTalk_1
                            $ display = holder[1]
                            $ OverrideSleeping = 0
                            if skillChoice.descrips == "PlayWhilePlayerSleeping":
                                $ OverrideSleeping = 1
                ...
                if(skillChoice.skillType == "statusEffect"):
                    if(skillChoice.statusChance > 0):
                        ...
                        if skillChoice.statusEffect == "Analyze":
                            $ defender.encyclopedia = "_Analyze"
                            $ stanceGo = "True"
                            ...
                            python:
                                ...
                                for res in effectResArray:
                                    if defender.resistancesStatusEffects.getRes(res) < 0:
                                        WeakArray.append(res)
                                    elif defender.resistancesStatusEffects.getRes(res) >= 150:
                        ...
                        if(stanceGo == "True"):
                            ...
                            if attacker.species != "Player":
                                ...
                                call onHitPreTalk(defender, attacker, skillChoice) from _call_onHitPreTalk_2
                            if skillChoice.removesStance[0] != "" and skillChoice.removesStance[0] != "None":
                                python:
                                    for removeTheStance in skillChoice.removesStance:
                                        if removeTheStance != "All" and removeTheStance != "Target":
                        ...
                        if  possibleOptions == monSkillChoice[m].name and onlyOnce == 0:
                            ...
                            if canGo == 1:
                                ...
                                if display[-1] != "|":
                                    display = display + " "
    if display != "":
        call read from _call_read_50
    $ display = ""
    $ display = premonDisplay
    $ premonDisplay = ""
    $ Speaker = Character(_(attacker.name + " - " + nameExtra))
    return
...
label PlayerLossCheck:
    $ display = ""
    if checkSPLosses(orgasmTarget) == "Lost":
        $ Speaker = Character(_(''))
...
label MonsterLossCheck:
    ...
    while monLossCheck < len(monsterEncounter) and player.stats.sp > 0:
        ...
        $ out = 0
        $ show = 0
        if checkSPLosses(monsterEncounter[monLossCheck]) == "Lost":
    ...
    python:
        def OrgasmCheck(theTarget, attacker, move):
            ...
            if checkHPLosses(theTarget, attacker, move) == "Orgasm":
                ...
                spiritLost = SpiritCalulation(theTarget, spiritLost, 1, edging, ignoreOrgasm)
        ...
        def SpiritCalulation(theTarget, SpiritLoss=1, eventapplicable=0, edging=0, ignoreOrgasm=0):
            for perk in theTarget.perks:
                p = 0
                while  p < len(perk.PerkType):
                    if perk.PerkType[p] == "MultiplySpiritLoss":
                        SpiritLoss *= perk.EffectPower[p]
                    if edging == 0 and ignoreOrgasm == 0:
                        if perk.PerkType[p] == "RemovedOnOrgasm":
                            theTarget.giveOrTakePerk(perk.name, -1)
                            break
                    p += 1
            ...
            if renpy.random.randint(0,100) + attackerAcc + move.accuracy >= TargetsEvade - surrenderMod - restrainedMod - fetishMod:
                ...
                if move.statusOutcome != "IgnoreAttack":
                    ...
                    if effectivenessTotal >= 1.25:
                        effectiveText = "{i}{color=#ff587d}Weakspot!{/color}{/i} "
                    elif effectivenessTotal <= 0.75:
                        effectiveText = "{i}{color=#535F98}Frigid!{/color}{/i} "
                    ...
                    if critChance >= critRoll:
                        ...
                        Crit = 1
                        critText = "{i}{color=#ff587d}Passionate!{/color}{/i} "
                    else:
                        Crit = 0
                    ...
                    if (theTarget.statusEffects.defend.duration > 0):
                        ...
                        for perk in theTarget.perks:
                            ...
                            while  p < len(perk.PerkType):
                                ...
                                p += 1
                        if theTarget.statusEffects.defend.potency == 3:
                            finalDamage *= 0.25-DefendBonus
                    ...
                    display = move.outcome + " " + "[EffectiveText]" + "[CritText]" + "[TargetName] is aroused by " + "[FinalDamage]!" + " {i}[StatusEffectiveText]{/i}"
        ...
        def applySleepy(afflicted):
            global finalSleepy
            sleepyDamage = afflicted.statusEffects.sleep.potency
            sleepyAmp = 1
            for perk in afflicted.perks:
                p = 0
                while  p < len(perk.PerkType):
                    if perk.PerkType[p] == "SleepAmp":
                        sleepyAmp += (perk.EffectPower[p])*0.01
                    p += 1
            if afflicted.species == "Player":
                if difficulty == "Hard":
                    sleepyAmp += 0.25
            sleepyDamage *= sleepyAmp
            finalSleepy = int(math.floor(sleepyDamage))
            afflicted.stats.ep -= finalSleepy
            return afflicted
        ...
        def statusBuff(user, attacker, move, autoHits= 0): #user is the target, attacker is the one using the ability.
            ...
            if move.statusPotency >= 0 and move.statusEffect != "Escape" and move.statusEffect != "TargetStances":
                ...
                if move.statusEffect == "Escape" or move.statusEffect == "TargetStances":
                    EffectChance = getStatusEffectChance(move.statusChance, user, attacker, move, autoHits)
                else:
        ...
        def statusAfflict(theTarget, move, attacker=Monster(Stats(), 0, "FAKEtARGET")):
            ...
            if move.statusEffect == "Restrain":
                ...
                restraintMaxHP = theTarget.statusEffects.restrained.duration
        ...
        def getHealingEstimate(healer, move):
            ...
            if move.statType == "PercentMaxEnergy":
                finalHealing = healer.stats.max_true_ep*(finalHealing*0.01)

            elif move.statType == "PercentMaxArousal":
                finalHealing = healer.stats.max_true_hp*(finalHealing*0.01)

            elif move.statType == "Max Arousal":
                finalHealing += ((healer.stats.max_true_hp-100)*0.1)*scaling
                finalHealing *= ((healer.stats.max_true_hp-100)*0.001)*scaling + 1
            elif move.statType != "None":
                relatedStat = healer.stats.getStat(move.statType)*scaling
                finalHealing += int(relatedStat)

            itemBonus = 1
            if move.isSkill != "True" and finalHealing > 0:
                for perk in healer.perks:
                    p = 0
                    while  p < len(perk.PerkType):
                        if perk.PerkType[p] == "ItemBonus":
                            itemBonus += (perk.EffectPower[p])*0.01
                        p += 1

            finalHealing = int(math.floor(finalHealing*itemBonus))
        ...
        def HealCalc(attacker, move):
            ...
            if(move.skillType != "StatusHeal"):
                Hit = 1
                finalHealing = getHealingEstimate(attacker, move)
                finalHealing *= (renpy.random.randint(move.minRange, move.maxRange)*0.01) #random modifyer to damage
                finalHealing = int(math.floor(finalHealing))
                ...
                if move.isSkill != "True":
                    ...
                    if itemChoice.hp != 0:
                        if itemChoice.hp < 0:
                            healingArousal += int(math.floor(itemChoice.hp))
                        else:
                            healingArousal += int(math.floor(itemChoice.hp*itemBonus))
                    if itemChoice.ep != 0:
                        if itemChoice.ep > 0:
                            healingEnergy += int(math.floor(itemChoice.ep))
                        else:
                            healingEnergy += int(math.floor(itemChoice.ep*itemBonus))
                    if itemChoice.sp != 0:
                        if itemChoice.sp > 0:
                            healingSpirit += int(math.floor(itemChoice.sp))
                        else:
                            healingSpirit += int(math.floor(itemChoice.sp*itemBonus))
                    if itemChoice.Exp != 0:
                        ExpGained += itemChoice.Exp

                if(move.skillType == "Healing"):
                    healingArousal += finalHealing
                if(move.skillType == "HealingEP"):
                    healingEnergy += finalHealing
                if(move.skillType == "HealingSP"):
                    healingSpirit += finalHealing

            if(healingArousal != 0):
                attacker.stats.hp -= healingArousal
                if healingArousal > 0:
                    display += " Arousal lowered by " + str(abs(healingArousal)) + "! "
                else:
                    display += " Arousal increased by " + str(abs(healingArousal)) + "! "
            if(healingEnergy != 0):
                attacker.stats.ep += healingEnergy
                if healingEnergy > 0:
                    display += " Recovered " + str(abs(healingEnergy)) + " energy! "
                else:
                    display += " Lost " + str(abs(healingEnergy)) + " energy! "
            if(healingSpirit != 0):
                attacker.stats.sp += healingSpirit
                if healingSpirit > 0:
                    display += " Recovered " + str(abs(healingSpirit)) + " spirit! "
                else:
                    display += " Lost " + str(abs(healingSpirit)) + " spirit! "
            if(ExpGained != 0):
                attacker.stats.Exp += ExpGained
                if healingSpirit > 0:
                    display += " Gained " + str(abs(ExpGained)) + " exp! "
                else:
                    display += " Lost " + str(abs(ExpGained)) + " exp! "
            if move.skillType == "Afflict":
                ...
                if move.statusEffect != "None"  or (move.isSkill != "True" and itemChoice.statusEffect != "None") :
                    ...
                    if move.statusEffect == "Aphrodisiac" or (move.isSkill != "True" and itemChoice.statusEffect == "Aphrodisiac"):
                        ...
                        elif move.isSkill != "True" and finalHealing <= 0:
                    ...
                    if move.statusEffect == "Paralysis" or (move.isSkill != "True" and itemChoice.statusEffect == "Paralysis"):
                        if attacker.statusEffects.paralysis.potency > 0:
                            if move.isSkill != "True":
                                attacker.statusEffects.paralysis.potency -= itemChoice.statusPotency
                            else:
                                attacker.statusEffects.paralysis.potency -= move.statusPotency
                            ...
                            if attacker.statusEffects.paralysis.potency <= 0:
                                ...
                                display += " You shiver a little, and the paralysis loosens its hold on you!"
