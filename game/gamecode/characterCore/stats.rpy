...
init python:
    class CombatStance(renpy.store.object):
        def __init__(self, Stance="None", potency=0, maxPotency=0):
            self.Stance = Stance
    ...
    class StatusEffects:
        ...
        trance=StatusEffect(), fascinated=StatusEffect(), confusion=StatusEffect(), paralysis=StatusEffect(), ahegao=StatusEffect()):
            ...
            #these three arent real and are placeholders.
        ...
        def hasThisStatusEffect(self, Name):
            ...
            if Name == "Buffs":
                for each in self.tempAtk:
                    if each.duration > 0 and each.potency > 0:
                        return True
                for each in self.tempDefence:
                    if each.duration > 0 and each.potency > 0:
                        return True
    ...
    def clearBuff(characterArray):
        b = 0
        while b < len(characterArray):
            if characterArray[b].duration > 0 and characterArray[b].potency > 0:
                del characterArray[b]
                b -= 1
            b += 1
        return characterArray

    def clearDebuff(characterArray):
        b = 0
        while b < len(characterArray):
            if characterArray[b].duration > 0 and characterArray[b].potency < 0:
                del characterArray[b]
    ...
    def clearStatBuff(characterArray, playerstat):
        b = 0
        while b < len(characterArray):
            if characterArray[b].duration > 0 and characterArray[b].potency > 0:
                playerstat -= characterArray[b].potency
                del characterArray[b]
                b -= 1
            b += 1
        return [characterArray, playerstat]

    def clearStatDebuff(characterArray, playerstat):
        b = 0
        while b < len(characterArray):
            if characterArray[b].duration > 0 and characterArray[b].potency < 0:
                playerstat -= characterArray[b].potency
    ...
    def removeThisStatusEffect(effect, character):
        ...
        if effect == "Buffs":
            character.statusEffects.tempAtk = clearBuff(character.statusEffects.tempAtk)
            character.statusEffects.tempDefence = clearBuff(character.statusEffects.tempDefence)

            holder = []
            holder  = clearStatBuff(character.statusEffects.tempPower, character.stats.Power)
            character.statusEffects.tempPower = holder[0]
            character.stats.Power = holder[1]
            holder = clearStatBuff(character.statusEffects.tempTech, character.stats.Tech)
            character.statusEffects.tempTech = holder[0]
            character.stats.Tech = holder[1]
            holder = clearStatBuff(character.statusEffects.tempInt, character.stats.Int)
            character.statusEffects.tempInt = holder[0]
            character.stats.Int = holder[1]
            holder = clearStatBuff(character.statusEffects.tempWillpower, character.stats.Willpower)
            character.statusEffects.tempWillpower = holder[0]
            character.stats.Willpower = holder[1]
            holder = clearStatBuff(character.statusEffects.tempAllure, character.stats.Allure)
            character.statusEffects.tempAllure = holder[0]
            character.stats.Allure = holder[1]
            holder = clearStatBuff(character.statusEffects.tempLuck, character.stats.Luck)
            character.statusEffects.tempLuck = holder[0]
            character.stats.Luck = holder[1]

            character.statusEffects.tempCrit = clearBuff(character.statusEffects.tempCrit)

        if effect == "Debuffs":
            character.statusEffects.tempAtk = clearDebuff(character.statusEffects.tempAtk)
            character.statusEffects.tempDefence = clearDebuff(character.statusEffects.tempDefence)
            holder = clearStatDebuff(character.statusEffects.tempPower, character.stats.Power)
            character.statusEffects.tempPower = holder[0]
            character.stats.Power = holder[1]
            holder = clearStatDebuff(character.statusEffects.tempTech, character.stats.Tech)
            character.statusEffects.tempTech = holder[0]
            character.stats.Tech = holder[1]
            holder = clearStatDebuff(character.statusEffects.tempInt, character.stats.Int)
            character.statusEffects.tempInt = holder[0]
            character.stats.Int = holder[1]
            holder = clearStatDebuff(character.statusEffects.tempWillpower, character.stats.Willpower)
            character.statusEffects.tempWillpower = holder[0]
            character.stats.Willpower = holder[1]
            holder = clearStatDebuff(character.statusEffects.tempAllure, character.stats.Allure)
            character.statusEffects.tempAllure = holder[0]
            character.stats.Allure = holder[1]
            holder = clearStatDebuff(character.statusEffects.tempLuck, character.stats.Luck)
            character.statusEffects.tempLuck = holder[0]
            character.stats.Luck = holder[1]
            character.statusEffects.tempCrit = clearDebuff(character.statusEffects.tempCrit)
    ...
    class Skill:
        def __init__(self, name="blank", costDisplay="0",  costType="ep", skillType="attack", statType="", skillTags=[], fetishTags=[],
            ...
            self.unusableIfStatusPotency = [int(potency) for potency in self.unusableIfStatusPotency]
        ...
        def __ne__(self, other):
            ...
            equal = self.CompareArray(self.unusableIfStatusPotency, other.unusableIfStatusPotency, equal)
        ...
        def updateSkill(self):
            try:
                ...
                setattr(self, 'unusableIfStatusPotency', [0])
    ...
    class Fetish(renpy.store.object):
        def __init__(self, name = "", Level=0, Type = "", CreationOn = "", CreationOff = "", toolTip = "", levelText=[]):
            self.name = name
            self.Level = Level
            self.Type = Type
